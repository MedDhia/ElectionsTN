...
| the paper count | `papers_certified == 1` | 9,198 (97.4%) |
...
| ...total backed by the ballots column too | `valid_corroborated == 1` | 9,085 (96.2%) |
...
| value | rows | meaning |
|---|---|---|
| `1` | 9,085 | the ballots column agrees; `valid` is vouched for by two independent identities |
...
| empty | 333 | nothing to check against: 302 certified rows whose ballots column is not published, so `valid` rests on the votes identity alone, plus the 31 rows with no certified votes at all |
